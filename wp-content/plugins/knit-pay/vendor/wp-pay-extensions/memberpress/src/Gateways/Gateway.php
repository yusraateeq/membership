...
class Gateway extends MeprBaseRealGateway {
	...
	public function process_payment( $transaction ) {
		...
		$payment = $this->set_phone( $payment, $transaction );
	...
	private function set_phone( $payment, $txn ) {
		if ( empty( $this->settings->phone_slug ) ) {
			return $payment;
		}
		...
		if ( isset( $phone ) ) {
			$payment->get_customer()->set_phone( $phone );
			$payment->get_billing_address()->set_phone( $phone );
		}
